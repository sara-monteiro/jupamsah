...
function fazerRequisicao(tipo, id, nome, email, cpf, matricula, cargo, localtrabalho) {
    ...
    fetch(url, {method: 'get'}).then(function(response){
        ...
        for (var i = 0; i < cadadmin.length; i++) { 
            linhas += `<tr>` +
                        `<td>${cadadmin[i].id}</td>` +
                        `<td>${cadadmin[i].nome}</td>`+ 
                        `<td>${cadadmin[i].email}</td>` +
                        `<td>${cadadmin[i].cpf}</td>` +
                        `<td>${cadadmin[i].matricula}</td>` +
                        `<td>${cadadmin[i].cargo}</td>` +
                        `<td>${cadadmin[i].localtrabalho}</td>` +
                      `</tr>`;
        }
